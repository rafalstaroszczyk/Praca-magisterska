...
W zagadnieniach niezmienniczych z obrotem ważnym zagadnieniem jest moment pędu cząstki. Uwględnia on zarówno ten związany z ruchem \(\vu{L} = \vu{r} \times \vu{p}\), jak i spin związany z samą cząstką \(\vu{S}\). W ogólności możemy go oznaczyć jako \(\vu{J}\), będący sumą wcześniejszych. Operator ten jest wektorowym, to znaczy, że każda z jego składowych jest operatorem \(\hat{J}_x, \hat{J}_y, \hat{J}_z\). Spełniają one własności komutacyjne:
...
\begin{figure}[!ht]
	...
	\caption{Wykres zależności wybranych wielkości termodynamicznych od temperatury dla układu składającego się z cząstki o spinie \(\inv{2}\).}
...
\begin{align}\label{eq:spin_pol_E}
	E_{\uparrow}   &= - \frac{\gamma B \hbar}{2} = - \frac{\hbar\omega}{2}, &
	E_{\downarrow} &=   \frac{\gamma B \hbar}{2} =   \frac{\hbar\omega}{2}.
\end{align}

W równowadze termodynamicznej \cite{kieu, yuan}:
\begin{gather}
	Z = \exp\pab{\frac{\hbar\omega}{2 k_B T}} + \exp\pab{-\frac{\hbar\omega}{2 k_B T}} = 2\cosh\pab{\frac{\hbar\omega}{2 k_B T}}, \label{eq:spin_pol_Z} \\
	p_{m_s} = \inv{Z} \exp\pab{-\frac{E_{m_s}}{k_B T}}, \label{eq:spin_pol_pk} \\
	U = p_{\uparrow}E_{\uparrow} + p_{\downarrow}E_{\downarrow} = - \frac{\hbar\omega}{2} \tgh\pab{\frac{\hbar\omega}{2 k_B T}}, \label{eq:spin_pol_U} \\
	S = -k_B\pab{p_{\uparrow} \ln p_{\uparrow} + p_{\downarrow} \ln p_{\downarrow}} =
	- \frac{\hbar\omega}{2T} \tgh\pab{\frac{\hbar\omega}{2 k_B T}} + k_B \ln\bab{2\cosh\pab{\frac{\hbar\omega}{2 k_B T}}}. \label{eq:spin_pol_S}
\end{gather}
