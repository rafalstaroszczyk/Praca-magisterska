...
Podczas cyklu Otto ciepło jest wymieniane jedynie w przemianach izochorycznych. Szybkość przepływu ciepła jest proporcjonalna do różnicy temperatur między gazem roboczym a zbiornikiem ciepła zgodnie z prawem Fouriera \cite{endo_otto}:
\begin{align}
...
Równanie \eqref{eq:otto_stosunek_T} w formie:
\begin{equation*}\label{eq:otto_endo_stosunek_T}
	\frac{T_1}{T_2} = \frac{T_4}{T_3} = \inv{r^{\varkappa-1}} = \kappa.
\end{equation*}

Z powyższych wzorów oraz \eqref{eq:otto_endo_stosunek_T} możemy wyprowadzić wzory na temperaturę zależne od \(\tau_h\), \(\tau_c\) oraz \(\kappa\):
...
Sprawność cyklu Otto w punkcie maksymalnej mocy jest identyczne z przypadkiem równowagowym i zależne jedynie od \(\kappa\):
\begin{equation}
	\eta = 1 - \inv{r^{\varkappa}} = 1 - \kappa.
\end{equation}
...
\end{multline}
Moc jest stosunkiem pracy w jednym cyklu do długości tego cyklu \(\tau = \zeta\pab{\tau_h + \tau_c}\):
\begin{equation}
...
Wyrażenie na sprawność zależy jedynie od \(\kappa\), natomiast na moc można rozseparować na czynnik zależny od \(\kappa\) i od pozostałych zmiennych. Aby znaleźć sprawność w punkcie maksymalnej mocy wystarczy znaleźć maksimum funkcji zależnej od \(\kappa\):
\begin{gather*}
