\documentclass[10pt,twoside]{report}
\usepackage{config/preamble}

\usepackage{csquotes}
\usepackage[sorting=none,giveninits=true]{biblatex}
\addbibresource{config/bibliography.bib}

\begin{document}

\pagenumbering{gobble}  % Do not number the first pages

% A title page generated from mojaPG
% \includepdf[pages=-]{pdf/title-page.pdf}
% \null\newpage

% An author statement generated from mojaPG
% \includepdf[pages=-]{pdf/author-statement.pdf}
% \null\newpage

%\include{misc/abstract-pl.tex}
%\newpage
%\null\newpage

%\include{misc/abstract-en.tex}
%\newpage
%\null\newpage

\pagenumbering{arabic}  % Start numbering on ToC
\tableofcontents

%\include{misc/list-of-symbols.tex}

% Include chapters one-by-one
%\include{chapters/01.tex}
\chapter{Termodynamika klasyczna}
\input{chapters/Silniki_klasyczne.tex}
\input{chapters/Twierdzenie_Carnot.tex}
\input{chapters/Twierdzenie_Clausius.tex}
\input{chapters/Endo_Carnot.tex}
\input{chapters/Endo_Otto.tex}
\input{chapters/Endo_Stirling.tex}
\chapter{Mechanika kwantowa}
%\input{chapters/Macierz_gestosci.tex}
\input{chapters/Stany_czyste_i_mieszane.tex}
\input{chapters/Kwantowe_maszyny_cieplne.tex}
\input{chapters/Kwantowy_Otto.tex}
\input{chapters/Kwantowy_Stirling.tex}
\appendix
\input{chapters/Wyprowadzenie_entropii_kwantowej.tex}
\input{chapters/Studnia_potencjalu.tex}
\input{chapters/Oscylator_harmoniczny.tex}
\input{chapters/Moment_pedu.tex}
\input{chapters/Cialo_dwustanowe.tex}
\input{chapters/Kod.tex}

\nocite{*}
\printbibliography[title={Wykaz literatury},heading=bibintoc]

\listoffigures
\addcontentsline{toc}{chapter}{Wykaz rysunków}

\listoftables
\addcontentsline{toc}{chapter}{Wykaz tabel}

\input{figures/3dplot.tex}

\end{document}
