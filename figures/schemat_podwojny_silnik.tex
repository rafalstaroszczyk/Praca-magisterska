...
\begin{tikzpicture}[x=0.5cm, y=0.5cm]
	...
	\node[anchor=east] at (-\engsep-\engwidth, 0.5*\tanksep) {\(Q_{hX}\)};
	\node[anchor=east] at (-\engsep-\engwidth, -0.5*\tanksep) {\(Q_{cX}\)};
	...
	\node[anchor=west] at (\engsep+\engwidth, 0.5*\tanksep) {\(Q_{hC}\)};
	\node[anchor=west] at (\engsep+\engwidth, -0.5*\tanksep) {\(Q_{cC}\)};
	\draw[line width=0.5mm, -{Stealth[length=5mm, open]}] (\engsep, 0.75*\tanksep) -- (\engsep, 0.25*\tanksep);
	\draw[line width=0.5mm, -{Stealth[length=5mm, open]}] (\engsep, -0.75*\tanksep) -- (\engsep, -0.25*\tanksep);
	
	\draw[black] (-\engsep+\engwidth+\radius, -\chansep) -- (\engsep-\engwidth-\radius, -\chansep);
	\draw[black] (-\engsep+\engwidth+\radius, \chansep) -- (\engsep-\engwidth-\radius, \chansep);
	\node[anchor=south] at (0, \chansep) {\(W\)};
