\begin{tikzpicture}[x=0.5cm, y=0.5cm]
	...
	\node[anchor=center] at (-\engsep, 0.5*\tanksep) {\(\frac{\eta_C}{\eta_X}Q\)};
	\node[anchor=center] at (-\engsep, -0.5*\tanksep) {\(\pab{\frac{\eta_c}{\eta_X}-\eta_C}Q\)};
	...
	\node[anchor=center] at (\engsep, 0.5*\tanksep) {\(Q\)};
	\node[anchor=center] at (\engsep, -0.5*\tanksep) {\(\pab{1-\eta_C}Q\)};
	
	\draw[black] (-\engsep+\engwidth+\radius, -\chansep) -- (\engsep-\engwidth-\radius, -\chansep);
	\draw[black] (-\engsep+\engwidth+\radius, \chansep) -- (\engsep-\engwidth-\radius, \chansep);
	\node[anchor=center] at (0, 0) {\(\eta_C Q\)};
\end{tikzpicture}