\tikzsetnextfilename{pv_silnik_endo_carnot_70}
\begin{tikzpicture}
	\begin{axis}[width=0.7\textwidth,
		...
		\addplot[domain=V2:V1] {1/r * tau^(-kappa/(kappa-1)) / x}             node [pos=0.3, below, sloped] {$T_{cw}$};
		\addplot[domain=V3:V2] {1/r^kappa * tau^(-kappa/(kappa-1)) / x^kappa} node [pos=0.5, below, sloped] {$Q=0$};
		\addplot[domain=V3:V4] {1/r * tau^(-1/(kappa-1)) / x}                 node [pos=0.5, above, sloped] {$T_{hw}$};
		...
		\draw[line width=0.3mm, -{Stealth[length=5mm, open]}] (0.75,0.1) -- (0.75,0.025) node[anchor=south west]{$Q_{1-2}$};
		
		\addplot[domain=V2:V1, dashed] {0.5*1/r * tau^(-kappa/(kappa-1)) / x}             node [pos=0.1, below, sloped] {$T_c$};
		\addplot[domain=1.1*V3:1.1*V4, dashed] {1.3*1/r * tau^(-1/(kappa-1)) / x}                 node [pos=0.5, above, sloped] {$T_h$};
	\end{axis}
\end{tikzpicture}
